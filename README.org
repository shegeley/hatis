...
** TODO Catch clipboard (~wl_data_control/source~ + ~wl_primary_selection~)
...
---
Try:
1. ~open-file~  with mode ~for-output-and-append~ and then parametrize ~current-output-port~ and pass the port from ~open-file~

2. will run ~guile~ and function that need to be executed; ~guile -L src -c @@((hatis wayland) main)~. this way it can be started/stopped from terminal.
