...
it will cause "silent error": it will be written in a console, that there are no capability, but no exception will be thrown, so it's not possible to catch it with
~(with-exception-handler ...)~. and it will, in fact "return" ~wl-touch~ object. althought it will be "broken".

workaround is to always ~(extract-capabilities ...)~ and only "do things" after getting the
