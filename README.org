...
By default wayland doesn't have ability to explicitly "distingush" one input context from another and also access what's already [[https://wayland.app/protocols/input-method-unstable-v2#zwp_input_method_v2:request:commit_string][commited]] into text-input.
...
It would be nice to save "unfinished" text-input. See [[id:8fce6ad3-309c-4354-bffc-dab11f37c6cc][TODO]]. And not only from one text-input field, but from all of them. But how do I distinguish them?... There is no ID given to each... Maybe [[id:6759d3ce-3027-452e-b95c-d420b89d7394][popup surface]]?
...
The usecase in unclear yet

Options:
1. (ice-9 peg)
2. tree-sitter grammar: [[https://crates.io/crates/xkb-parser/0.1.0/dependencies][xkb-parser]] (rust :()
...
:END:
** TODO Try distinguish text-input context via it's popup surface
:PROPERTIES:
