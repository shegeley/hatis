...
- IDE: Emacs + [[https://git.sr.ht/~abcdw/guile-ares-rs][Ares]]/[[xhttps://git.sr.ht/~abcdw/emacs-arei][Arei]]
- OS: Guix System
...
* Notes
** guile-nyacc C bindings autogenerator
@krevedkokun gave a hint on autogenerating C bindings with guile-scheme with nyacc. [[https://git.sr.ht/~krevedkokun/guile-wlroots/tree/master/item/ffi/wlroots.ffi][See]].
It it possible to entirely replace guile-wayland and forget about handwritten bingings mess?
** ~(swaymsg -t get_tree) -> scm~ dump example
#+begin_src scheme
(("sticky" . #f)
 ("fullscreen_mode" . 0)
 ("focus" . #(3))
  ...
  #((("sticky" . #f)
     ...
     ("nodes"
      ...
      #((("sticky" . #f)
         ...
         ("window" . null)
         ("name" . "__i3_scratch")
         ("geometry" ("height" . 0) ("width" . 0) ("y" . 0) ("x" . 0))
         ("window_rect" ("height" . 0) ("width" . 0) ("y" . 0) ("x" . 0))
         ("deco_rect" ("height" . 0) ("width" . 0) ("y" . 0) ("x" . 0))
         ("rect" ("height" . 1036) ("width" . 1920) ("y" . 0) ("x" . 0))
     ...
     ("id" . 2147483647))
    (("max_render_time" . 0)
     ("current_mode" ("refresh" . 0) ("height" . 1036) ("width" . 1920))
     ("current_workspace" . "1")
      ...
      #((("representation" . "H[foot foot]")
         ...
         ("nodes"
          ...
          #((("idle_inhibitors" ("application" . "none") ("user" . "none"))
             ...
             ("window" . null)
             ("name" . "foot")
             ("geometry" ("height" . 485) ("width" . 700) ("y" . 0) ("x" . 0))
             ("window_rect"
              ("height" . 986)
              ("width" . 956)
              ("y" . 0)
              ("x" . 2))
             ("deco_rect" ("height" . 25) ("width" . 960) ("y" . 0) ("x" . 0))
             ("rect" ("height" . 988) ("width" . 960) ("y" . 48) ("x" . 0))
            ...
            (("idle_inhibitors" ("application" . "none") ("user" . "none"))
             ...
             ("deco_rect"
              ...
              ("y" . 0)
              ("x" . 960))
             ("rect" ("height" . 988) ("width" . 960) ("y" . 48) ("x" . 960))
             ("current_border_width" . 2)
             ("border" . "normal")
             ("layout" . "none")
             ("focused" . #t)
         ...
         ("window_rect" ("height" . 0) ("width" . 0) ("y" . 0) ("x" . 0))
         ("deco_rect" ("height" . 0) ("width" . 0) ("y" . 0) ("x" . 0))
         ("rect" ("height" . 1013) ("width" . 1920) ("y" . 23) ("x" . 0))
         ("current_border_width" . 0)
         ("border" . "none")
         ("layout" . "splith")
         ("focused" . #f)
 ...
 ("id" . 1))
#+end_src
How do I get all "windows"? Top level "type" is "root". "output" type is nested in it. And then comes the "workspace" and THEN "con" which is a "console" in the exact given example.

Or it's also possible to recursively get all nodes untill there is not nested nodes on the one that's left...

It's also possible to use a jq (fast commandline json processor).

I need the results to strictly correspond to <zlwr-foreign-toplevel...> protocol abilities: getting title (change) and app-id (change) and forcing the focus (activating? ~<request name="activate">~?)
---
For now I did it with jq system* calls and pipes. It's temporary anyway untill guile-wayland's toplevel-management is fixed.

Draft from "better attempt" (parsing swaymsg-tree with guile-wayland)
#+begin_src scheme
(define (get-focused x)
 (let* [(p (open-input-pipe swaymsg:get-tree-command*))
        (json-string (get-string-all p))]
  (close-port p)
...
---
↑ [[id:476bea70-feb2-46b3-b3a7-217b66046ee0][done]] ~(@@ (hatis wayland wrappers) make-listener)~
...
I don't yet understand wayland's client loop in full details. My current understanding is based on [[id:518ab312-dd50-41e8-b922-55b419adbd38][examples]].
...
Order of the last 2 steps are still suspisious to me. What is "do-something" is an of itself and endless loop?
---
Conceptually:
#+begin_src scheme
...
** TODO Get "window manager context" (see definitions) (~app_id~ + ~pid~ & etc) [high priority] [swaymsg or [[https://wayland.app/protocols/wlr-foreign-toplevel-management-unstable-v1#zwlr_foreign_toplevel_handle_v1:event:app_id][zwlr_foreign_toplevel_handle_v1:event:app_id]] (hard)]
...
#+begin_verse
master 53b3676b8bc1128f7d159a2aee00526046951ebf
Author:     Grigory Shepelev <shegeley@gmail.com>
AuthorDate: Sun Aug 4 23:32:17 2024 +0300
Commit:     Grigory Shepelev <shegeley@gmail.com>
CommitDate: Sun Aug 4 23:32:17 2024 +0300

Parent:     b40c1d0 util: define-wl-{server,client}-procedure: Set name.
Merged:     master
Contained:  master
Follows:    v0.0.2 (33)

(modules wayland scanner) allow-null -> allow-null? [wild guess]

1 file changed, 2 insertions(+), 2 deletions(-)
modules/wayland/scanner.scm | 4 ++--

modified   modules/wayland/scanner.scm
@@ -99,7 +99,7 @@
             (arg (@ (type ,arg-type)
                     (name ,(_->- -> arg-name))
...
---
DONE [[https://github.com/guile-wayland/guile-wayland/issues/13][Create a reprodusible example + issue and send to ~guile-wayland~]]
---
See [[https://github.com/swaywm/wlroots/blob/0855cdacb2eeeff35849e2e9c4db0aa996d78d10/examples/foreign-toplevel.c#L256C2-L259C3][wlroots toplevel example]]:
#+begin_src c
...
#+end_src
---
UPD: last (on <2024-08-27 Вт>) Z572's [[https://github.com/guile-wayland/guile-wayland/commit/cf96a8a5d710addb330021c1d3b21738447b2efb][commi]]t seems to relate, but won't do anything.
---
UPD: the problem is not even in the #:toplevel event, but in ~<…-toplevel-manager…>~ itself. Once inited via ~registry:init-interface~, even without event-listener it will crush the programm on (roundtrip) or (dispatch) with SIGSEGV.
---
What I've tried:
...
Posted on mastodon that I'm having troubles with guile + gdb. One guy [[https://fosstodon.org/deck/@ramin_hal9001@emacs.ch/113034248882817135][poste]]d a vague advice:
...
I also [[https://forum.systemcrafters.net/t/scheme-guile-debugging/484/3][rant]] about the situation with the lack of guile + gdb examples in SystemCrafters forum.
---
Did more experimentation with GDB.
...
#+end_src
---
On <2024-08-31 Сб> Z572 [[https://github.com/guile-wayland/guile-wayland/issues/13#issuecomment-2322868090][confirmed the bug]] and wrote smallest possible reprodusible example.

Then I thought the problem might be in versioning tried to build the latest wlroots and the latest sway with it. No help.

Tried once more to do some dirty tricks with minimal reprodusible: wrapping and unwrapping, minor changes in guile-wayland.

Then looked at ~guile-wlroots~. Found nothing usefull cause it's for creating wayland-servers.
---
At this point (<2024-09-03 Вт>) I have to drop idea of using this protocol untill it's fixed by someone else. I just can't waste more time on it.

Will use ~swaymsg~ with ~system*~ and ~(ice-9 popen)~. It can show the "wayland tree" (scene graph or how does it called?) but also to change focus on request.
...
#+end_quote
** DONE Write a custom listener constructor that'd fill all the default events
:PROPERTIES:
:ID:       476bea70-feb2-46b3-b3a7-217b66046ee0
:END:
~(@@ (hatis wayland wrappers) make-listener)~
** DONE Add proper stop (+restart should be as a bonus) ability
...
---
Casting ~(wl-display-disconnect (i <wl-display>))~ throws 139 (memory) error. It need proper shutdown. THINK
---
Figured out:
#+begin_src scheme
(define (exit!)
